...
class SecureStorageHelper {
  ...
  Future<String?> read({required String key}) async {
    try {
      ...
      log.e('SecureStorage read failed for key "$key"', error: e);
      return null;
    }
  }
  ...
  Future<void> write({required String key, required String? value}) async {
    try {
      ...
      log.w('SecureStorage write failed for key "$key", retrying', error: e);
      try {
        await _storage.delete(key: key);
        await _storage.write(key: key, value: value);
      } catch (e2) {
        log.e('SecureStorage write retry failed for key "$key"', error: e2);
      }
    }
  }
  ...
  Future<void> delete({required String key}) async {
    try {
      ...
      log.e('SecureStorage delete failed for key "$key"', error: e);
    }
  }
  ...
  Future<Map<String, String>> readAll() async {
    try {
      ...
      log.e('SecureStorage readAll failed', error: e);
      return {};
    }
  }
}
